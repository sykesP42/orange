  ...
  <div class="detail-page" v-if="blogger">
    <div class="page-header">
      ...
      <div class="header-right">
        ...
        <button class="invalid-btn" @click="confirmSetInvalid" v-if="canEdit && !blogger.is_invalid">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="rejectReasonVisible" class="modal-overlay" @click.self="cancelReject">
      <div class="dialog-content" style="max-width:440px;">
        <h3>确认拒绝该博主？</h3>
        <p>请填写拒绝原因，并选择是否将博主移入失效库。</p>
        ...
        <div style="margin-top:16px;display:flex;gap:12px;justify-content:flex-end;">
          ...
          <button class="btn btn-primary" @click="confirmReject" style="background:#ef4444;">确认拒绝并移入失效库</button>
        </div>
      </div>
    </div>
        ...
        <div class="similar-bloggers-list">
          ...
          >
            <div class="similar-avatar">
              ...
              <template v-else>{{ sb.nickname?.charAt(0) || '?' }}
    <!-- Reject Reason Dialog -->
    <div v-if="rejectReasonVisible" class="modal-overlay" @click.self="cancelReject">
      <div class="dialog-content" style="max-width:440px;">
        <h3>确认拒绝该博主？</h3>
...
// Watch status change for reject dialog
watch(() => editForm.value.status, (newStatus, oldStatus) => {
  if (newStatus === '已拒绝' && oldStatus !== '已拒绝') {
    pendingRejectStatus.value = newStatus
    rejectReasonVisible.value = true
    // Revert status temporarily until confirmed
    editForm.value.status = oldStatus || '初次联系'
  }
})

const confirmSetInvalid = async () => {
  if (!await confirmDanger('确定要将该博主加入失效库吗？加入后该博主将被标记为失效状态。')) return
  try {
    const res = await setInvalidBloggerAPI(blogger.value.id)
    if (res.code === 200) {
      notification.success('已加入失效库')
      blogger.value.is_invalid = 1
    } else {

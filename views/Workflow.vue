        ...
        <div class="dialog-content">
          ...
          <div class="form-group" v-if="form.action_type === 'notification'">
            ...
            <div class="config-fields">
              ...
              <textarea v-model="form.actionConfig.content" placeholder="通知内容，支持变量：{nickname}（自动替换为博主昵称）" rows="3"></textarea>
              <select v-model="form.actionConfig.priority">
                <option value="normal">普通优先级</option>
                <option value="important">重要</option>
                <option value="urgent">紧急</option>
              </select>
              <p class="hint-text">通知将自动发送给该博主的负责人</p>
            </div>
          </div>
...
const toggleRule = async (rule) => {
  try {
    ...
    if (res.code === 200) {
      const wasEnabled = rule.enabled === 1
      rule.enabled = wasEnabled ? 0 : 1
      notification.success(wasEnabled ? '规则已禁用' : '规则已启用')
    }
  } catch (error) {
...
const editRule = (rule) => {
  ...
  try {
    ...
    form.actionConfig = { title: '', content: '', priority: 'normal', tag_ids: [], new_status: '' }
  }

...
const loadBloggerCount = async () => {
  try {
    const data = await bloggerStatAPI()
    if (data.code === 200) {
      bloggerCount.value = data.data?.stats?.total || 0
    }
  } catch (error) {
    // ignore
  }
...
const handleExport = async (type) => {
  try {
    ...
    notification.success('导出成功！', `已导出 ${selectedFieldCount.value} 个字段`)
  } catch (error) {
  ...
  try {
    ...
    if (userStore.username === 'admin') {
      ...
      if (settingsData.code === 200) {
        backupInterval.value = settingsData.data.backup_interval || 86400000
      }
    }
  } catch (error) {

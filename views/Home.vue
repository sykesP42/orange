<template>
  <div class="home">
    ...
    <TagCloud
      v-if="allTags.length > 0 && !isMobile"
      :tags="tagCloudData"
    ...
    <div v-if="!loading && bloggers.length === 0" class="empty-state">
      ...
      <div class="empty-actions">
        ...
        <button class="btn-secondary" @click="router.push('/admin')">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
import { bloggerListAPI, bloggerStatAPI, categoryListAPI, getPublicUsersAPI, bloggerDeleteAPI, getBloggerStatusList, getTeamsAPI, getAnnouncementsAPI, getInvalidBloggerCountAPI, getTagsAPI, batchUpdateStatusAPI, batchUpdateTagsAPI, batchDeleteAPI, getSavedFiltersAPI, createSavedFilterAPI, deleteSavedFilterAPI, bloggerUpdateAPI } from '../api'
...
const loadStats = async () => {
  try {
    ...
    if (res.code === 200 && res.data?.stats) {
      todayNew.value = res.data.stats.today || 0
      weekGrowth.value = res.data.stats.week || 0
    }
  } catch (error) {
    logger.error('', error)
  }
...
const getStatusColor = (status) => {
  ...
  return colorMap[status] || 'var(--info)'
}

<template>
  <div class="team-page">
    ...
    <div v-if="!userStore.team_id" class="admin-actions">
    ...
    <div v-if="teams.length === 0 && !loading" class="empty-state">
      ...
      <p>暂无团队</p>
      <button v-if="!userStore.team_id" class="btn btn-primary" @click="openCreateTeam">创建第一个团队</button>
    </div>
...
const saveTeam = async () => {
  ...
  if (formData.value.name.trim().length > 30) {
    formError.value = '团队名称不能超过30个字符'
    return
  }
  if (formData.value.description && formData.value.description.length > 200) {
    formError.value = '团队描述不能超过200个字符'
    return
  }
  ...
  try {
    ...
    if (res.code === 200) {
      success(isEditing.value ? '团队更新成功' : '团队创建申请已提交，请等待管理员审核')
      closeFormPanel()
      if (isEditing.value) loadTeams()
    } else {
...
.btn-primary:hover {
  background: var(--bg-brand-gradient-hover);

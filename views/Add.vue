    ...
    <form @submit.prevent="handleSubmit" class="form-card">
      ...
      <div class="form-group">
        ...
        <div class="input-wrapper">
          ...
          <select v-model="form.category" required :disabled="optionsLoading">
            <option value="">{{ optionsLoading ? '加载中...' : '选择分类' }}</option>
      ...
      <div class="form-group">
        ...
        <div class="input-wrapper">
          ...
          <select v-model="form.product" required :disabled="optionsLoading">
            <option value="">{{ optionsLoading ? '加载中...' : '选择产品' }}</option>
      ...
      <div class="form-group">
        ...
        <div class="input-wrapper">
          ...
          <select v-model="form.platform" required :disabled="optionsLoading">
            <option value="">{{ optionsLoading ? '加载中...' : '选择平台' }}</option>
...
const loadPlatforms = async () => {
  try {
    ...
    logger.error('', error)
    notification.error('加载平台失败')
  }
}

const handleSubmit = async () => {
  form.nickname = form.nickname?.trim() || ''
  form.platform_account = form.platform_account?.trim() || ''
  form.contact = form.contact?.trim() || ''
  form.wechat = form.wechat?.trim() || ''
  form.custom_contact = form.custom_contact?.trim() || ''
  ...
  if (form.nickname.length > 30) {
    notification.warning('博主昵称不能超过30个字符')
    return
  }
  if (form.platform_account.length > 50) {
    notification.warning('平台账号不能超过50个字符')
    return
  }
  if (form.contact && !/^1[3-9]\d{9}$/.test(form.contact) && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.contact)) {
    notification.warning('联系方式格式不正确，请输入手机号或邮箱')
    return
  }
  if (form.wechat && !/^[a-zA-Z][a-zA-Z0-9_-]{5,19}$/.test(form.wechat)) {
    notification.warning('微信号格式不正确（6-20位，字母开头）')
    return
  }
  if (form.description && form.description.length > 500) {
    notification.warning('描述不能超过500个字符')
    return
  }
  if (form.tags.length > 10) {
    notification.warning('标签数量不能超过10个')
    return
  }
...
onMounted(async () => {
  ...
  await Promise.all([loadCategories(), loadProducts(), loadPlatforms()])

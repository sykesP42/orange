...
let rafId = null
let timeColor = { bg1: '#0f0c29', bg2: '#302b63', bg3: '#24243e', accent: '#f97316', accentAlt: '#ec4899', gridAlpha: 0.03 }

function getTimeTheme() {
  const h = new Date().getHours() + new Date().getMinutes() / 60
  if (h >= 5 && h < 7) return {
    bg1: '#1a1428', bg2: '#2d1b3d', bg3: '#1e1528',
    accent: '#fbbf24', accentAlt: '#f97316',
    gridAlpha: 0.025, orbAlpha: 0.04, particleAlpha: [0.03, 0.08],
    name: 'dawn'
  }
  if (h >= 7 && h < 10) return {
    bg1: '#1c1410', bg2: '#3d2518', bg3: '#261a10',
    accent: '#fb923c', accentAlt: '#f97316',
...
class AmbientOrb {
  ...
  draw(ctx, theme) {
    ...
    grad.addColorStop(0, color === '#f97316' || color === '#fb923c' || color === '#f59e0b' || color === '#fbbf24'
      ? `rgba(249,115,22,${this.currentAlpha})` : this.hexToRgba(color, this.currentAlpha))
    grad.addColorStop(0.5, this.hexToRgba(color, this.currentAlpha * 0.3))
  ...
  for (let i = 0; i < particles.length; i++) {
    for (let j = i + 1; j < particles.length; j++) {
      ...
      if (dist < 120) {
        ...
        ctx.strokeStyle = theme.accent.startsWith('#')
...
const handleLogin = async () => {
  ...
  try {
    ...
    else notification.error(res.message || '登录失败')
  } catch (error) { notification.error(error.response?.data?.message || '登录失败，请检查网络') }
...
.form-actions a:hover { color: #f97316; }
